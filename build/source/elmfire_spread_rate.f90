...
! *****************************************************************************
SUBROUTINE UMD_UCB_BLDG_SPREAD(C)
! *****************************************************************************
...
INTEGER :: I, TEMP_SX, TEMP_SY, DEL_X, DEL_Y, IX_LOW_BORDER, IY_LOW_BORDER, IX_HIGH_BORDER, IY_HIGH_BORDER, &
           IX_TEMP, IY_TEMP, HAZ
REAL :: DFC_COEFF, RAD_COEFF, FTP_PA, ANALYSIS_CELLSIZE_SQUARED, TOTAL_DFC, TOTAL_RADIATION, RAD_PER_SQCELL, &
        UCB_DIV, DFC_FACTOR, DFC_HEAT_RECEIVED, RAD_LIMIT_THETA, RAD_CHECKER, DELTA_RAD, RAD_FACTOR, &
...
TOTAL_DFC = TOTAL_DFC_WUI(C%IX, C%IY)
TOTAL_RADIATION = TOTAL_RADIATION_WUI(C%IX, C%IY)
...
IX = BURNING_NODES%IX
IY = BURNING_NODES%IY
TOA = TIME_OF_ARRIVAL(BURNING_NODES%IX,BURNING_NODES%IY)
IF (TOA .EQ. -9999.) RETURN

BURNING_TIME = T - TOA

IF (BURNING_NODES%IFBFM .NE. 91) THEN
   IF (BURNING_TIME .LT. ANALYSIS_CELLSIZE/BURNING_NODES%VELOCITY_DMS_SURFACE)
...
! *****************************************************************************
SUBROUTINE CALC_WUI_HEATFLUX(BURNING_NODES, T)
! *****************************************************************************
! Calculate the emitted heat flux from BURNING_NODES to adjcent cells (IX,IY)
USE ELMFIRE_VARS
...
LHRR_PEAK = BUILDING_FUEL_MODEL_TABLE(BURNING_NODES%IBLDGFM)%HRRPUA_PEAK
...
HRR_ADJUSTER = ANALYSIS_CELLSIZE_SQUARED/(PI*BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MAJOR*BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MINOR)  !DWI_M2
ELLIPSE_MINOR_SQUARED = BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MINOR * BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MINOR
...
DO IX = IXTAGSTART, IXTAGSTOP
   DEL_X = IX - BURNING_NODES%IX
   DEL_Y = IY - BURNING_NODES%IY
   ...
   MAX_ELLIPSE_DIST = 0.3 * BURNING_NODES%ELLIPSE_PARAMETERS%DIST_DOWNWIND * (BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MAJOR - BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_ECCENTRICITY) / ELLIPSE_MINOR_SQUARED

   ! Direct Flame Contact
   ELLIPSE_DIST_THETA = MAX_ELLIPSE_DIST*ELLIPSE_MINOR_SQUARED / (BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_MAJOR - BURNING_NODES%ELLIPSE_PARAMETERS%ELLIPSE_ECCENTRICITY*COS(TARGET_THETA_F))
   ...
   DFC_HEAT_RECEIVED = DFC_COEFF*DFC_FACTOR*BURNING_NODES%HRR_TRANSIENT*HRR_ADJUSTER  !DWI_M3
   ...
   RAD_HEAT_RECEIVED = HRR_ADJUSTER*(0.3*DFC_COEFF*RAD_COEFF*RAD_FACTOR*BURNING_NODES%HRR_TRANSIENT*ANALYSIS_CELLSIZE_SQUARED)/(4*PI*RAD_EFF_DIST*RAD_EFF_DIST)  !DWI_M4

   HEAT_VALUE = HEAT_VALUE + (DFC_HEAT_RECEIVED + RAD_HEAT_RECEIVED)*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED

   TOTAL_DFC_WUI(IX,IY) = TOTAL_DFC_WUI(IX,IY) + DFC_HEAT_RECEIVED*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED
   TOTAL_RADIATION_WUI(IX,IY) = TOTAL_RADIATION_WUI(IX,IY) + RAD_HEAT_RECEIVED*SIMULATION_DT*ANALYSIS_CELLSIZE_SQUARED
...
USE ELMFIRE_VARS
! FUEL_LOAD_REMAIN, HRR_TRANSIENT_MAP, TOTAL_RADIATION_WUI, TOTAL_DFC_WUI, TIME_OF_ARRIVAL, FBFM, CRITICL_HF_WUI
