...
IF (BURNING_NODES%IFBFM .NE. 91) WRITE(*,*) "CALLING HRR_TRANSIENT", TOA
IF (TOA .EQ. -1) RETURN
...
ELLIPSE_PARAMETERS = ELLIPSE_PROPERTY_MAP(BURNING_NODES%IX, BURNING_NODES%IY)
HRR_ADJUSTER = ANALYSIS_CELLSIZE_SQUARED/(PI*ELLIPSE_PARAMETERS%ELLIPSE_MAJOR*ELLIPSE_PARAMETERS%ELLIPSE_MINOR)  !DWI_M2
...
WRITE(*,*) "CALLING CALC_WUI_HEATFLUX", IXTAGSTART, IXTAGSTOP, IYTAGSTART, IYTAGSTOP
...
DO IX = IXTAGSTART, IXTAGSTOP
   ...
   IF (TRANSIENT_DFC_WUI(IX,IY) .GT. 0.) THEN
      WRITE(*,*) IX,IY, FBFM%I2(IX,IY,1), TRANSIENT_DFC_WUI(IX,IY), TRANSIENT_RADIATION_WUI(IX,IY)
   ENDIF
ENDDO
ENDDO
