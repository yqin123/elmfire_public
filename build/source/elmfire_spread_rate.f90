...
TOTAL_DFC = TOTAL_DFC_WUI(IX, IY)
TOTAL_RADIATION = TOTAL_RADIATION_WUI(IX, IY)
...
C%ABSOLUTE_U = 60*(TOTAL_DFC + TOTAL_RADIATION)/(0.3048*SIMULATION_DT*ANALYSIS_CELLSIZE*FTP_PA)/UCB_DIV ! Unit: ft/min
C%ABSOLUTE_U = MIN(C%ABSOLUTE_U, C%WS20_NOW)
...
! ENDIF
IF (BURNING_NODES%IFBFM .NE. 91) THEN
   BURNING_NODES%HRR_TRANSIENT = 250
   HRR_TRANSIENT_MAP(IX,IY) = BURNING_NODES%HRR_TRANSIENT
   RETURN
ENDIF
